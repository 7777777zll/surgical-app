<template>
  <div class="instrumentBoxList">
    ...
    <el-table ref="multipleTable" :data="tableData" border tooltip-effect="dark" style="width: 100%" @selection-change="handleSelectionChange" v-loading="isLoading" element-loading-text="拼命加载中">
      ...
      <el-table-column prop="registerDate" label="注册时间" width="120" show-overflow-tooltip>
      </el-table-column>
      <el-table-column label="柜子状态" show-overflow-tooltip>
        <template slot-scope="scope">{{ formatStopFlag(scope.row.stopFlag) }}</template>
      </el-table-column>
      <el-table-column label="操作" width="200">
        <template slot-scope="scope">
          <el-button type="warning" class="mr-10" size="small" @click="handleExamine(scope.$index, scope.row)">审核</el-button>
        </template>
      </el-table-column>
    </el-table>
    <el-col :span="24" class="queryBar">
...
<script>
  ...
  export default {
    ...
    methods: {
      ...
      onSubmit () {
        // this.isLoading = true
        instrumentBoxList.waitExamine({params: Object.assign(this.page, this.formInline)})
          .then(response => {
      ...
      handleExamine (index, row) {
        ...
        params.cabinetState = 1
        instrumentBoxList.examine(params.id)
          .then(response => {
            this.$message({
              showClose: true,
              message: '审核成功',
