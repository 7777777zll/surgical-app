<template>
  <div class="producerDict">
    ...
    <el-table ref="multipleTable" :data="tableData" border tooltip-effect="dark"  v-loading="isLoading" element-loading-text="拼命加载中">
      ...
      <el-table-column prop="repairCount" label="维修（把/次）" width="200" show-overflow-tooltip>
      </el-table-column>
      <el-table-column prop="overhallCount" label="保养（把/次）" width="200"  show-overflow-tooltip>
      </el-table-column>
      <el-table-column prop="scrapCount" label="报废(把)" width="200"  show-overflow-tooltip>
      </el-table-column>
    </el-table>
    <el-col :span="24" class="queryBar">
...
<script>
  import { report, instrumentDict } from '../../api/api'
  export default {
    data () {
      return {
        // 时间选择设置
      ...
      onSubmit () {
        ...
        this.formInline.endTime = this.formInline.endTime
          ? this.moment(this.formInline.endTime)
              ...
              .format('YYYY-MM-DD')
          : null
        report.op({ params: Object.assign(this.page, this.formInline) }).then(
          response => {
